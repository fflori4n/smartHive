...
#define TOFFS -3
#define ADC_TO_CURRENT(ARG) (ARG * (140/25.3))
...
void loop() {
  ...
  current = (0.95*current) + (0.05 * ADC_TO_CURRENT(analogRead(CURRENT_PIN)));
  voltage = (0.95*voltage) + (0.05 * ADC_TO_VOLTAGE(analogRead(VOLTAGE_PIN)));
  light = ADC_TO_LIGHT(analogRead(LIGHT_PIN));
  temp = (temp * 0.98) + (0.02*ADC_TO_DEGC(analogRead(TEMP_PIN)));

  delay(CYCLE_TIME);
  loopCount++;
  if(loopCount > SEND_PERIOD/CYCLE_TIME){
    ...
    Serial.println((27.0/140.0) * current);
