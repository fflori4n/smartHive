...
int sendMqttSIM(SIM7000& gsmModem,char(& mqttPayloadBuff)[MQTT_PAYLOAD_BUFF_LEN], const char* mqttTopic){
 ...
 /* if(gsmModem.atSend("AT+CNACT?\r","OK", 5000, "0.0.0.0") != 0){
  ...
  if(gsmModem.atSend("AT+SMCONF?\r","sandorr.eunetddns.net") != 0){ /// List Mqtt Settings, if URL matches, params are loaded into config from previous loop, do not config again
    gsmModem.atSend("AT+SMCONF=\"URL\",sandorr.eunetddns.net,1883\r","OK"); /// List Mqtt Settings
