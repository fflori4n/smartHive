...
#define POLL_ALL_LEN (sizeof(POLL_ALL)/ sizeof(char) - 1)
  ...
  public:
    ...
    */
    ...
    /*void sendDataPullMsg(char deviceType, char deviceID){

      snprintf(msgBuff, MSGBUFF_LEN, "AF,POL*");
      uint16_t crc = crc16(msgBuff, strlen(msgBuff));
      //inStr = START_BYTE + inStr + STOP_BYTE;
      snprintf(msgBuff, MSGBUFF_LEN, "<<<AF,POL*%c%c>>>",((char*) &crc)[1]),((char*) &crc)[0];
      //Serial.println(msgBuff);
      printBuff();
      //sendBuffStr();
      printToRS485Serial(strlen("AF,POL*") + 9);
    }*/

    /*int sendStr() {
      
      uint16_t crc = crc16(inStr, inStr.length());
      inStr += ((char*) &crc)[1];
      inStr += ((char*) &crc)[0];
      inStr = START_BYTE + inStr + STOP_BYTE;
    ...
    void sendPollRequest(){
      ...
      snprintf(msgBuff, MSGBUFF_LEN, "AF,POL*");
      uint16_t crc = crc16(msgBuff, strlen(msgBuff)-1);
      if(!((strlen(msgBuff)+5 + 3) < MSGBUFF_LEN)){ /// +3 because msg will be shifted right 3 places
        Serial.println(F("No space in buffer!"));
        return;
      }
      uint8_t lenMsgBuf = strlen(msgBuff);
      msgBuff[lenMsgBuf +5] = '\0';
      msgBuff[lenMsgBuf +4] = '>';
      msgBuff[lenMsgBuf +3] = '>';
      msgBuff[lenMsgBuf +2] = '>';
      msgBuff[lenMsgBuf +1] = (char)(crc | 0xFF);
      msgBuff[lenMsgBuf] = (char)(crc >> 8);
      shiftBuffRight(3, '<');
      ...
      //sendBuffStr();
      printToRS485Serial(lenMsgBuf + 9);
    }

    void printToRS485Serial(uint8_t outStrLen){
      digitalWrite(13, HIGH);
      digitalWrite(DE_PIN, HIGH); /// Driver enabled - send
      delayMicroseconds(5000);
